...
export const getStats = query({
  ...
  handler: async (ctx) => {
    ...
    const jobs = await ctx.db.query("jobPostings").withIndex("by_status", (q) => q.eq("status", "Open")).collect();
    const apps = await ctx.db.query("applications").collect();
    const activeApps = apps.filter((a) => !["Rejected", "Withdrawn", "Hired"].includes(a.stage));
    const filledJobs = await ctx.db.query("jobPostings").withIndex("by_status", (q) => q.eq("status", "Filled")).collect();
    const avgDays = filledJobs.length > 0
      ...
      : 0;
    const interviews = await ctx.db.query("interviews").withIndex("by_status", (q) => q.eq("status", "Completed")).collect();
    let pendingEvals = 0;
...
export const getUpcomingInterviews = query({
  ...
  handler: async (ctx) => {
    ...
    const interviews = await ctx.db.query("interviews").withIndex("by_scheduledDate").order("asc").collect();
